...
<body class="min-h-screen flex bg-[#FDFBF7] text-[#333333] font-sans antialiased">
    ...
    <aside class="hidden md:flex md:flex-col md:w-64 md:shrink-0 border-r border-[#E2E8F0] bg-[#FAFAFF] px-4 py-6">
        <a href="{{ route('company.dashboard') }}" class="text-xl font-semibold tracking-tight text-[#1E293B] px-2 mb-8">
            Dipla <span class="text-[#4A3B5C] font-normal text-sm">Pro</span>
        </a>

        <nav class="flex flex-col gap-1 text-sm font-medium">
            <a href="{{ route('company.dashboard') }}"
               class="rounded-full px-4 py-2.5 transition {{ request()->routeIs('company.dashboard') ? 'bg-[#1E3D59] text-[#FDFBF7]' : 'text-[#1E293B] hover:bg-[#FDFBF7]' }}">
                Tableau de bord
            </a>
            <a href="{{ route('company.products.index') }}"
               class="rounded-full px-4 py-2.5 transition {{ request()->routeIs('company.products.*') ? 'bg-[#1E3D59] text-[#FDFBF7]' : 'text-[#1E293B] hover:bg-[#FDFBF7]' }}">
                Mes produits
            </a>
        </nav>

        <form method="POST" action="{{ route('company.logout') }}" class="mt-auto">
            @csrf
    ...
    <nav id="company-menu-mobile" class="hidden md:hidden fixed top-14 inset-x-0 z-20 flex-col gap-1 bg-[#FAFAFF] border-b border-[#E2E8F0] px-4 py-3 text-sm font-medium">
        <a href="{{ route('company.dashboard') }}" class="block rounded-full px-4 py-2.5 {{ request()->routeIs('company.dashboard') ? 'bg-[#1E3D59] text-[#FDFBF7]' : 'text-[#1E293B]' }}">Tableau de bord</a>
        <a href="{{ route('company.products.index') }}" class="block rounded-full px-4 py-2.5 {{ request()->routeIs('company.products.*') ? 'bg-[#1E3D59] text-[#FDFBF7]' : 'text-[#1E293B]' }}">Mes produits</a>
