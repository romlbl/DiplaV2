{{-- resources/views/company/products/_image-queue.blade.php --}}
<div x-data="productImageQueue()">
    <label class="block text-sm font-medium text-[#1E293B] mb-1">Photos (jusqu'à 4)</label>
    <p class="text-xs text-[#333333]/60 mb-2">Recadre chaque photo au format 2:3, puis réorganise-les par glisser-déposer.</p>
    ...
    <div data-role="sortable-images"
        ...
        <template x-for="item in items" :key="item.id">
            <div :data-id="item.id" class="relative group cursor-move">
                <img :src="item.previewUrl" alt="" class="aspect-[2/3] w-full object-cover rounded-xl border border-[#E2E8F0]">
                <button type="button" @click="confirmingDelete = item.id"
    ...
    <label x-show="items.length < 4"
           class="inline-flex items-center gap-2 rounded-full border border-[#E2E8F0] px-4 py-2 text-sm font-medium text-[#1E293B] cursor-pointer hover:bg-[#FDFBF7] transition">
        + Ajouter des photos
    ...
    <div x-show="confirmingDelete !== null" x-cloak
        ...
        <div @click.outside="confirmingDelete = null"
            ...
            <div class="flex gap-3 mt-5">
                ...
                <button type="button" @click="removeItem(confirmingDelete); confirmingDelete = null"
